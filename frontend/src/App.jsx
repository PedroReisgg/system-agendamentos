...
export default function App(){
 ...
 const[page,setPage]=useState(rota),[empresa,setEmpresa]=useState(padrao),[servicos,setServicos]=useState([]),[usuario,setUsuario]=useState(null),[modal,setModal]=useState(false),[mode,setMode]=useState('login'),[busy,setBusy]=useState(false),[erro,setErro]=useState('');
 ...
 return <>{page==='agendar'?<Booking supabase={supabase} tenant={{name:empresa?.nome,config_json:empresa?.configuracao_json}} services={servicosFront} user={usuario} onHome={()=>ir('inicio')} onAccount={()=>abrir()} onLogout={sair}/>:<Landing tenant={{name:empresa?.nome||padrao.name,config_json:empresa?.configuracao_json||padrao.config_json}} onBook={()=>ir('agendar','#agendar')} onAccount={()=>abrir()} user={usuario&&{name:usuario.nome,role:usuario.papel}} onLogout={sair} onDashboard={()=>ir('dashboard','#dashboard')} onProfile={irPerfil} onBooking={()=>ir('agendar','#agendar')}/>} {modal&&<AccountModal mode={mode} setMode={setMode} onClose={()=>setModal(false)} onLogin={d=>autenticar('login',d)} onRegister={d=>autenticar('register',d)} services={servicos.map(s=>({...s,name:s.nome}))} busy={busy} error={erro}/>}</>;
}
